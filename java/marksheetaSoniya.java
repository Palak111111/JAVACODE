...
class MarksPOJO
{
	...
	private double per;
	public MarksPOJO(String name,String fname,String mname,String course,String branch,int roll,int year,int sem,int p,int c,int m,int h,int e,int total,double per)
	{
		this.name=name;
		this.fname=fname;
		this.mname=mname;
...
class Test
{
	...
	public static void input()
	{
		...
		{			
		    ...
		    year=Test.inputYear(s,i,sc);
			System.out.println("Enter your sem");
			sem=sc.nextInt();
			sc.nextLine();
			mark=Test.inputMarks(s,i,sc);
	
			s[i]= new MarksPOJO(name,fname,mname,course,branch,roll,year,sem,mark,total,per);
		    s[i].setTotal();
			s[i].setPercentage();
	    }
	...
	public static int inputRoll(MarksPOJO s[],int i,Scanner sc)
	{
		boolean y=false;
		int roll;
		do
